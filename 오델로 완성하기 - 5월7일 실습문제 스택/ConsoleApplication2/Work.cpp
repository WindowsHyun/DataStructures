...
enum{ None, Black, White, Blank, tBlack, tWhite, tempRBlack, tempRWhite };
...
int main(){
	...
	while(1){
		...
		printf("Real Coordinates ( %d , %d )", tempX, tempY-1 );
...
void isDrawBWstones(int line){
	...
	for(i=0; i<8; ++i){
		if( arr[line][i] == None ){
			...
			if( arr[line][i] == tBlack || arr[line][i] == tWhite || arr[line][i] == tempRBlack || arr[line][i] == tempRWhite ){
				if( arr[line][i] == tBlack)
					printf("★");
				if( arr[line][i] == tWhite)
					printf("☆");

				if( arr[line][i] == tempRBlack)
					printf("♥");
				if( arr[line][i] == tempRWhite)
					printf("♡");
...
void isClearTempBW(){
	for(int i=0; i<8; ++i){
		for(int j=0; j<8; ++j){
			if(arr[i][j] == tempRBlack||arr[i][j] == tempRWhite){
				arr[i][j] =  None;
			}
		}
	}
...
int isTempReverseStones(){
	for(k=-1; k<=1; ++k){
		for(l=-1;l<=1; ++l){
			...
			if(arr[isTempRy+k][isTempRx+l] != None){
				if( isPlayUser == Black){
					//검정색일경우 itPlayUser
